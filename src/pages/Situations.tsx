...
const situations = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "Le travail a permis de poser ce que personne n'avait jamais dit : le CEO voulait que les autres prennent plus de décisions. Les autres le voulaient depuis longtemps — mais ils avaient arrêté d'y croire. Restait à en faire un fonctionnement concret : qui décide quoi, qui est impliqué dans quel groupe, comment les décisions se prennent au quotidien.",
    ],
  },
  {
    number: "03",
...
const Situations = () => {
  return (
    <SiteLayout>
      <section>
        <div className="mx-auto max-w-6xl px-6 md:px-10 pt-16 md:pt-28 pb-16 md:pb-24">
          <p className="text-xs uppercase tracking-[0.3em] text-accent mb-10">
            Situations
          </p>
          <h1 className="font-serif text-5xl md:text-7xl leading-[1.05] text-balance text-foreground max-w-4xl">
            Trois histoires, trois manières dont une dynamique se dénoue.
          </h1>
          <p className="mt-10 text-lg text-muted-foreground max-w-2xl leading-relaxed">
            Les noms et détails ont été modifiés. Ce qui demeure&nbsp;: ce qui
            s'est joué, et ce qui a permis à chacun de retrouver sa
            capacité d'agir.
          </p>
        </div>
      </section>
      ...
      <section className="border-t border-border/60">
        <div className="mx-auto max-w-6xl px-6 md:px-10 divide-y divide-border/70">
          {situations.map((s) => (
            <article
              key={s.number}
              className="grid gap-10 lg:grid-cols-12 py-20 md:py-32"
            >
              <header className="lg:col-span-4 lg:sticky lg:top-32 lg:self-start">
                <p className="font-serif italic text-7xl md:text-8xl text-accent/90 leading-none">
                  {s.number}
                </p>
                <h2 className="mt-6 font-serif text-3xl md:text-4xl leading-[1.15] text-foreground text-balance">
                  {s.title}
                </h2>
              </header>
              <div className="lg:col-span-8 space-y-6 text-lg text-foreground/85 leading-relaxed">
                ...
                {s.pull && (
                  <p className="font-serif italic text-3xl text-accent pt-4">
                    {s.pull}
                  </p>
                )}
              </div>
      ...
      <CtaBlock heading="Votre situation a-t-elle un écho ici&nbsp;?" />
    </SiteLayout>
  );
};
